...
int cleanupAndIdentify(SDL_Surface* digitSurface) {
    // The biggest blob is usually the border around it, we remove it and then find the next biggest blob (the digit)
    BiggestBlob_result borderBlob = findBiggestBlob(digitSurface);
    floodFill(digitSurface, borderBlob.point, WHITE, BLACK);
    SDL_FreeSurface(borderBlob.res); // We don't need the SDL_Surface generated by findBiggestBlob()
